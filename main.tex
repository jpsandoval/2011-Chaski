...
We propose \emph{execution blueprints} to visually identify execution patterns. Our blueprints offer intuitive execution representations, useful to identify abnormal execution.
Adequate combinations of layouts and coloring reveals certain execution anomalies and performances issues.

Thanks to our framework Chaski, we were able to find a number of serious defect in applications written in the Pharo programming language.
...
All our experiments and implementation have been carried out in Pharo\footnote{\url{http://pharo-project.org}}, a Smalltalk-dialect programming language. 
...
Clicking on a node triggers a structure pattern matching to identify similar execution. The message structure below Message 2 is searched over the complete trace. Two instances of this structure is found, indicated with \ct{A} and \ct{B}.
...
The computation invokes the same messages on 3 different objects. The interesting point here is the presence of the red nodes, which are 3 messages sent to the same objects. Identifying that an object receive several time the same messages may indicate a candidate for inserting a memoization. 
...
Messages are located in a large matrix. A message $m$ is at a position $(i,o)$ where $t$ the index of the message in the trace and $o$ is the object that received $m$. Message \ct{m1} is the first message, received by \ct{O1}. Executing the method that corresponds to \ct{m1} sends the message \ct{m2} to one of the instance of \ct{C3}. 

Packages, classes and objects are vertically sorted along the number of messages they have sent. A component that sends more messages than its sibling is located above.
...
The Execution Mural visualization shows time on the horizontal axis and messages passed between classes in a program on the vertical~\figref{ExecutionMuralDemo}. This take the approach of creating
 a general view showing all the classes and messages. Where the classes are on vertical axis, and each message is a vertical line of 1 pixel wide from source to destination class.
...
Mondrian renders an arbitrary domain by constructing a node for each domain element and draw edges to indicate relations between two domain elements. We identified two deficiencies in the execution of Mondrian. 
...
We noticed that it was consuming an abnormal amount of memory. This was perceived with excessing garbage collections. In Pharo the garbage collector operates by ``stopping the world'', making the application run slower than what is ought do. \ab{@@HERE}
...
In this paper we present two execution blueprints, with emphasizes in message sent and interactions between objects. 

Design an effective trace visualization that (1) is able to cope with these huge amounts of data, and (2) does not confuse the viewer, remains a challenge.

Our blueprints offer intuitive execution representations, useful to identify abnormal execution.  And with Chaski is easy define a new execution blueprint.

In the future we plan extend this approach to create more execution blueprints, considering: state related operation (intercept accesses to object state), and execution patterns. This last to improve the cognitive capability on the visualization.
