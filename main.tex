...
\ab{What is the order of the packages, objects and classes? Is the ordering reflects the order in the trace?}
\jp{ the (package, class, object) are sorted vertically by number of messages that has been sent, in this sense the component that sends more messages are located above.}
...
\ab{Are the message performed on O2 the same selector? If yes, how can I know this? Which selector is it?}
\jp{ Yes,  TGWhell>>returnHashOf: ..., I will add an inspect option on messages.}
...
The Execution Mural visualization shows time on the horizontal axis and messages passed between classes in a program on the vertical~\figref{ExecutionMuralDemo}. This take the approach of creating a general view showing all the classes and messages. Where the classes are on vertical axis, and each message is a vertical line of 1 pixel wide from source to destination class.
...
We used the execution blueprint to analyze Mondrian, an agile visualization engine. We identified two deficiencies in its executions.

Mondrian renders an arbitrary domain by constructing a node for each domain element an and edge for a relation between two domain elements. 
